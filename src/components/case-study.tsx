        ...
        <div className="grid grid-cols-1 gap-6 md:grid-cols-2">
          {cards.map((c) => (
            <Link
              key={c.label}
              href={c.href}
              className="group flex flex-col gap-5 rounded-xl border border-[#292927] bg-white/[0.03] p-8 transition-colors duration-200 hover:border-accent-400/40 hover:bg-white/[0.05]"
            >
              <span className="font-mono text-[12px] font-medium tracking-mono text-accent-400 uppercase">
                {c.label}
              </span>
              <h3 className="font-display text-[26px] leading-[1.2] font-semibold tracking-subhead text-[#fafaf9]">
                {c.title}
              </h3>
              <p className="text-[16px] leading-[1.6] text-[#a8a8a1]">{c.desc}</p>
              <span className="mt-auto text-[14px] font-medium text-accent-400 underline-offset-4 group-hover:underline">
                Read the case study →
              </span>
            </Link>
          ))}
